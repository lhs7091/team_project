...
public class HorseRacing implements GamePlay {
	...
	public void execute(Scanner sc, MemberDTO dto) {
		...
		System.out.println("2등일 경우 : 배당율*75%*배팅금액");
		System.out.println("3등일 경우 : 배당율*50%*배팅금액");
		System.out.println();
		
		percent(); //1~8번말 우승 확률 구하기
		showRank(); // 우승확률에 따른 랭킹 표시
		...
		boolean check = cu.noPoint(outPoint, dto);
		if(check) {
			return;
		}
		...
		//게임 결과 db에 기록
		dao.firstConnection(dto, "경마");
		resultGame(horse, outPoint, dto);
		return;
	...
	public void resultGame(int horse, int outPoint, MemberDTO dto) {
		int first;
		int second;
		int third;
		int money;
		...
		if(horse == first) {
			...
			money = dto.getGameMoney()+rewardPoint-outPoint;
			dao.gameWin(dto, rewardPoint-outPoint, "경마");
		}else if(horse == second){
			System.out.println("2등");
			rewardPoint = (int)(outPoint*batting[horse-1]*0.75);
			System.out.println("획득포인트 : "+rewardPoint);
			money = dto.getGameMoney()+rewardPoint-outPoint;
			dao.gameWin(dto, rewardPoint-outPoint, "경마");
		}else if(horse == third) {
