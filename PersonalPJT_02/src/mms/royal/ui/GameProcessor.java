...
public class GameProcessor { //게임하기 메뉴 선택시 진행
	...
	//멤버메서드
	public void menu(Scanner sc, MemberDTO dto) {
		int menu;
		boolean isStop = true;
		...
		MemberDTO newdto = sms.showMemberSvc(dto);
		if(newdto.getGameMoney()==0) {
			System.out.println("포인트가 부족합니다!! 충전해주세요.");
			return;
		}else {
			System.out.println("현재잔액 : "+newdto.getGameMoney()+"원");
		}
		...
		do {
			...
			if(gp != null) { //실제 수행할 Game을 넘겨주는 역할
				fc.gamePlayRequest(gp, sc, newdto);
			}
